...
int main(int argc, const char* argv[]) {
    ...
    int imgStartIndex =
        0;                 // first file index to load (assumes Lidar and camera names have identical naming convention)
    ...
    for (std::size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex += imgStepWidth) {
        ...
        assert(dataBuffer.size() <= dataBufferSize && "buffer exceeds its max size");
        ...
        float shrinkFactor = 0.1f;  // shrinks each bounding box by the given percentage to avoid 3D object merging at
                                    // the edges of an ROI
        ...
        bVis = false;
        if (bVis) {
            show3DObjects((dataBuffer.end() - 1)->boundingBoxes, cv::Size(4, 20), cv::Size(2000, 2000), true);
        }
        bVis = false;
        ...
        descKeypoints((dataBuffer.end() - 1)->keypoints, (dataBuffer.end() - 1)->cameraImg,
                      (dataBuffer.end() - 1)->descriptors, descriptorType);
        ...
        if (dataBuffer.size() > 1)  // wait until at least two images have been processed
        {
            ...
            //// STUDENT ASSIGNMENT
            //// TASK FP.1 -> match list of 3D objects (std::vector<BoundingBox>) between current and previous frame
            ...
            for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end();
                    ...
                    prevBB->lidarPoints.size() > 0)  // only compute TTC if we have Lidar points
                {
                    ...
                    std::cout << "TTC Lidar : " << ttcLidar << " s, TTC Camera : " << ttcCamera << " s" << std::endl;
                    ...
                    if (bVis) {
                        ...
                        cv::putText(visImg, oss.str(), cv::Point2f(80, 50), cv::FONT_HERSHEY_PLAIN, 2,
                                    cv::Scalar(0, 0, 255));
